...
class Login:
    def __init__(self, limit, timeout, users):
        ...
        for user in users:
            ...
            self.bduss = user['bduss']
#             if self.login() == False:
            if self.login_by_cookie(self.bduss) == False:
        ...
        if flag == False:
            ...
            exit(2)
    def login_by_cookie(self,bduss):
...
#             self.total = 0
            ...
            self.bduss = self.users[self.user_idx]["bduss"]
            print("move to next user: {0}".format(self.name))
            self.user_idx += 1
            if self.login_by_cookie(self.bduss) == True:
...
# #     bd.follow(users)
    ...
    bd.log.close()
#     login_by_cookie('VaUmdudUxvcHQyWmhOUnRMa3dPZHY0RTgwcWNMMDI4aXMwSElUbHdycTFCUTVYQVFBQUFBJCQAAAAAAAAAAAEAAACiVMwic2hlbnlhMTY4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAALV45la1eOZWTn')
